...
void DiscQueryHpxCoverageMapCellCount(std::string queryfile)
{
	ifstream ip;
	int i,j;
	std::vector<DiscType> discs;
	...
	if(ip.fail()){
	  cout << "Unable to open " << queryfile.c_str() << "!" << endl;
	  exit(1);     
	}
	discs = ReadRandomDiscs(ip);
	ip.close();

	MultiResHpx mMRH(12,NEST);
	Healpix_Base mHPX(12,NEST);

	cout << "Begin " << discs.size() << " Disc Queries..." << endl;
	for( i = 0; i < discs.size(); i++)
	{          
		// Do MRH Disc Query
		mMRH.QueryDisc(discs[i].pt,discs[i].radius);

		// Do HPX Disc Query
		mHPX.query_disc_inclusive(discs[i].pt,discs[i].radius,pixset);
		cout << "HPX Query Disc HPX Coverage Map Cell Count:," << pixset.size() << endl;	
	}
}

void PolyQueryHpxCoverageMapCellCount(std::string queryfile)
{
	ifstream ip;
	int i,j;
	std::vector<PolyType> polys;
	...
	{          
		// Do MRH Disc Query
		mMRH.QueryPolygon(polys[i].pts);

		// Do HPX Disc Query
		mHPX.query_polygon_inclusive(polys[i].pts,pixset,1);
		cout << "HPX Query Poly HPX Coverage Map Cell Count:," << pixset.size() << endl;	
	}
}
...
)
{
	...
	int MIN_INT,MAX_INT;
	int numFound = 0;
	int numAttempts,numGenQueries,backfillNum;
	...
	for( i = 0; i < numQueries; i++ ) 
	{
		// Do neighbor query to confirm at least one neighbor
		while( QUERYPASS == false && DONE == false) {
			// Random index draw
			index = MIN_INT + rand() % (MAX_INT-MIN_INT+1);
			
			// Check to see if already drew this random index
			if( !std::binary_search(searched.begin(),searched.end(),index) ) {
				searched.push_back(index);
				std::sort(searched.begin(),searched.end());
				n.pt = ms[index].pt;
				hpxid = mHPX.ang2pix(n.pt);
				...
				if( numFound >= minNeighbors ) {
					QUERYPASS = true;
				} 
			}
			// Tried every index, no matches
			if( searched.size() == ms.size() ) {
				DONE = true;
				QUERYPASS = false;
				i = numQueries;
				cout << "Unable to find required number of neighbor queries with minimum # neighbors. \nWriting out query file with queries that match and don't match criteriaand exiting." << endl;

			}
			numAttempts++;
			if( numAttempts%1000000 == 0 ) {
				cout << "Attempt " << numAttempts << ": " << n.pt.phi << ", " << n.pt.theta << endl;
			}

			if( numAttempts%100000000 == 0) {
				DONE = true;
			}
		}
		if( QUERYPASS == true ) {
			neighbors.push_back(n);
			QUERYPASS = false; 
			cout << "Query " << ++numGenQueries << " Found " << numFound << " neighbors" << endl; 
		}
	}

	// Back fill remaining required number of queries with valid neighbor queries that probably
	// won't meet minimum # neighors criteria
	if( numGenQueries < numQueries ) {
		backfillNum = (numQueries - numGenQueries);
		cout << "Back filling " << backfillNum << " neighbor queries." << endl;
		for(i = 0; i < backfillNum; i++) {
			// Random index draw
			index = MIN_INT + rand() % (MAX_INT-MIN_INT+1);
			n.pt = ms[index].pt;
			hpxid = mHPX.ang2pix(n.pt);
	...
	cout << "Total of " << numGenQueries << " being written out." << endl;
	...
	WriteRandomNeighborsHeader(fp,numGenQueries);
...
)
{
	...
	cf << "##### CONTENTS OF MRH MAP #####\n";
	for(unsigned int i = 0; i < mMRH.NumRec(); i++)
	{
		cf << mMRH[i].data1 << "\t"
			<< mMRH[i].data2 << "\t"
			<< mMRH[i].data3 << "\t"
	...
	// Save the MRH Map array to archive file
	cout << "Save the MRH Map array to archive file...\n\n";
	std::string outMapFile;
	outMapFile = outFile + ".map";
	mMRH.SaveMapToArchive(outMapFile);

	// Now save the internal MLQ forest to archive file
	cout << "Now save MRH data structure to archive file...\n\n";
	mMRH.SaveToFile(outFile);
	cout << "MRH data structure saved to archive file: " << outFile.c_str() << endl;


	// Now create a NEW MRH data structure constructed from
	// archive file!
	MultiResHpx_Map<Measurement> newMRH(maxDepthMRH,NEST);

	cout << "Now create NEW MRH data structure from archive file!\n";
	newMRH.LoadFromFile(outFile);

	// Now read in archived MRH Map array 
	mMRH.LoadMapFromArchive(outMapFile);
...
void MRHPointInsertionBenchMark(std::string dataInputFile, std::string outFile, int numTrials, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	...
	cout << "\nCompleted ALL Trials of " << numTrials*ms.size() << " Point Insertion in " << TotalTimeAllQuery 
		 << " ms! Average MRH Point Insertion: " << TotalTimeAllQuery/double(numTrials*ms.size()) << " ms\n";
...
void HPXPointInsertionBenchMark(std::string dataInputFile, std::string outFile, int numTrials)
{
	...
	// Report Results of All Trials
	cout << "\nCompleted ALL Trials of " << std::setprecision(20) << numTrials*ms.size() << " Point Insertion in " << TotalTimeAllQuery 
		 << " ms! Average HPX Point Insertion: " << TotalTimeAllQuery/double(numTrials*ms.size()) << " ms\n";
	fp << "\nCompleted ALL Trials of " << std::setprecision(20) << numTrials*ms.size() << " Point Insertion in " << TotalTimeAllQuery 
		 << " ms! Average HPX Point Insertion: " << TotalTimeAllQuery/double(numTrials*ms.size()) << " ms\n";
...
// same batch of queries multiple times. In this version (v2) will repeat each query numTrials of times
// so can see if any particular query is taking longer/shorter than averages. Will still end up with same
// final statistics as V1 but a bit more informative on a per-query basis.
void MRHDiscQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
	...
	for( i = 0; i < discs.size(); i++ )
	{
		...
		fp << "	Completed " << numTrials << " trials of Disc Query " << i+1 << " in " << TotalTimeCurQuery 
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
	}
...
void HPXDiscQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
...
void CompareDiscQueryOutput(std::string dataInputFile, std::string queryInputFile, std::string outFile, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
	int64 order,hpxid;
	int	totalMrhFound,totalHpxFound,totalMatches,totalMrhUnique,totalHpxUnique,totalMrhCritCount,totalHpxCritCount;
	...
	totalMrhFound = 0;totalHpxFound=0;totalMatches=0;totalMrhUnique=0;totalHpxUnique=0;totalMrhCritCount=0;totalHpxCritCount=0;
	for( i = 0; i < discs.size(); i++)
	{          
		...
		cout << "Query " << i+1 << " Matches: " << Matches << " MRH Only: " << MRHUnique << " HPX Only: " << HPXUnique 
			<< " MRH Crit Count: " << MRH_CRIT_COUNT << " HPX Crit Count: " << HPX_CRIT_COUNT << endl;
		fp << "Query " << i+1 << " Matches: " << Matches << " MRH Only: " << MRHUnique << " HPX Only: " << HPXUnique 
			<< " MRH Crit Count: " << MRH_CRIT_COUNT << " HPX Crit Count: " << HPX_CRIT_COUNT << "\n";
#else
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void MRHPolyQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
	...
	for( i = 0; i < polys.size(); i++ )
	{
		...
		cout << "	Completed " << numTrials << " trials of Poly Query " << i+1 << " in " << TotalTimeCurQuery 
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
		fp << "	Completed " << numTrials << " trials of Poly Query " << i+1 << " in " << TotalTimeCurQuery 
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
	}
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void HPXPolyQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials)
{
	ofstream fp;
	ifstream ip;
...
void ComparePolyQueryOutput(std::string dataInputFile, std::string queryInputFile, std::string outFile, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
	...
	{          
		...
		for( j=0; j < foundV.size(); j++ ) {
			if( mHPX[foundV[j]].rec != EMPTY ) {
				...
				// Lastly apply point-in-polygon test to filter out false positives
				if( IsPointInPoly2(p0proj,p) ){
					foundHPX.push_back(mHPX[foundV[j]]);
				}
			}
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void MRHStripQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
	...
	for( i = 0; i < strips.size(); i++ )
	{
		...
		cout << "	Completed " << numTrials << " trials of Strip Query " << i+1 << " in " << TotalTimeCurQuery 
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
		fp << "	Completed " << numTrials << " trials of Strip Query " << i+1 << " in " << TotalTimeCurQuery 
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
	}
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void HPXStripQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
...
void CompareStripQueryOutput(std::string dataInputFile, std::string queryInputFile, std::string outFile, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void MRHNeighborQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j;
	...
	for( i = 0; i < neighbors.size(); i++ )
	{
		...
		cout << "	Completed " << numTrials << " trials of Neighbor Query " << i+1 << " in " << TotalTimeCurQuery
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
		fp << "	Completed " << numTrials << " trials of Neighbor Query " << i+1 << " in " << TotalTimeCurQuery
			<< " ms! Average Query: " << avgQueryTime << " ms\n\n";
	}
...
// query bench mark to perform. This helps smooth out the average query estimation by repeating the
// same batch of queries multiple times.
void HPXNeighborQueryBenchMarkV2(std::string dataInputFile, std::string queryInputFile, std::string outFile, int numTrials)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
...
void CompareNeighborQueryOutput
(
 std::string dataInputFile, 
 std::string queryInputFile, 
 std::string outFile,
 int maxDepthMRH
)
{
	...
	{          
		...
		//cout << "Checking for neighbors of: \n" 
		//	 << neighbors[i].pt.phi << "," << neighbors[i].pt.theta << ","
		//	 << neighbors[i].pt.phi/pi << "," << cos(neighbors[i].pt.theta) << endl;
		AnalyzeResults(foundHPX,foundMRH,Matches,HPXUnique,MRHUnique,false);
		totalMatches += Matches;
		totalMrhUnique += MRHUnique;
		totalHpxUnique += HPXUnique;
...
// Want to quantify any possible differences with output of respective MRH and HPX neighbor queries
// given same data sets and same neighbor queries.
void CompareNeighborQueryOutputAtOrder
(
 std::string dataInputFile, 
 std::string queryInputFile, 
 std::string outFile,
 int64 maxBuildDepthMRH
)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
	int64 hpxid,measurements_order;
	int	totalMrhFound,totalHpxFound,totalMatches,totalMrhUnique,totalHpxUnique;
	int nMRHnodes,nHPXnodes;
	std::vector<Measurement> ms;
	std::vector<Measurement> foundMRH,foundHPX;
	...
	ms = ReadRandomMeasurements(ip,measurements_order);
	ip.close();
	...
	cout << "Begin adding " << ms.size() << " records to the MRH Data Structure...\n";
	fp << "Begin adding " << ms.size() << " records to the MRH Data Structure...\n";
	MultiResHpx_Map<Measurement> mMRH(maxBuildDepthMRH,NEST);
	fp << "Data Set:\n";
	fp << "Phi,Theta,P,Z\n";
	for( i = 0; i < ms.size(); i++)
	{
	...
	cout << "Begin adding " << ms.size() << " records to the HPX Data Structure...\n";
	fp << "Begin adding " << ms.size() << " records to the HPX Data Structure...\n";
	Healpix_Map<Measurement> mHPX(measurements_order,RING);
	for( i = 0; i < ms.size(); i++)
	{
		if( i%1000 == 0 ) {
			cout << "	Inserting Point " << i+1 << " of " << ms.size() << endl;
	...
	{          
		...
		// Do MRH Neighbor Query
		foundMRH = mMRH.Neighbors(neighbors[i].pt,maxBuildDepthMRH);
	...
	cout << "Memory Size of MRH: " << mMRH.SizeInMemKb() << " Kb" << " Size of HPX: " << (nHPXnodes*sizeof(Measurement))/1024 << " Kb" << endl;
	cout << "MRH Min Depth: " << minDepthMRH << " Max Depth: " << maxDepthMRH << " Avg Depth: " << avgDepthMRH << endl;
	cout << "HPX Depth: " << measurements_order << endl;
	cout << "Total Matches: " << totalMatches << endl;
	cout << "Total MRH Found: " << totalMrhFound << " Total MRH Unique: " << totalMrhUnique << endl;
	cout << "Total HPX Found: " << totalHpxFound << " Total HPX Unique: " << totalHpxUnique << endl;
	...
	fp << "Memory Size of MRH: " << mMRH.SizeInMemKb() << " Kb" << " Size of HPX: " << (nHPXnodes*sizeof(Measurement))/1024 << "Kb\n";
	fp << "MRH Min Depth: " << minDepthMRH << " Max Depth: " << maxDepthMRH << " Avg Depth: " << avgDepthMRH << "\n";
	fp << "HPX Depth: " << measurements_order << "\n";
	fp << "Total Matches: " << totalMatches << "\n";
	fp << "Total MRH Found: " << totalMrhFound << " Total MRH Unique: " << totalMrhUnique << "\n";
	fp << "Total HPX Found: " << totalHpxFound << " Total HPX Unique: " << totalHpxUnique << "\n";
...
void CompareDataStructures(std::string dataInputFile, std::string outFile, int maxDepthMRH)
{
	ofstream fp;
	ifstream ip;
	int i,j,k;
	...
	cout << "############################################" << endl;
	cout << "#### COMPARE MRH VS HPX DATA STRUCTURES ####" << endl;
	cout << "############################################" << endl << endl;

	fp << "############################################" << "\n";
	fp << "#### COMPARE MRH VS HPX DATA STRUCTURES ####" << "\n";
	fp << "############################################" << "\n" << "\n";
	...
	cout << "Begin adding " << ms.size() << " records to the HPX Data Structure...\n";
	fp << "Begin adding " << ms.size() << " records to the HPX Data Structure...\n";
	Healpix_Map<Measurement> mHPX(measurements_order,NEST);
	for( i = 0; i < ms.size(); i++)
	{
		if( i%1000 == 0 ) {
			cout << "	Inserting Point " << i+1 << " of " << ms.size() << endl;
	...
	// Write out list of MRH Data Points
	fp << "############################################\n";
	fp << "###### List of data point id's in MRH ######\n";
	fp << "############################################\n";
	fp << "Phi,Theta,P,Z,Rec\n";
	for( i = 0; i < mMRH.NumRec(); i++)
	{
 		// Write out data location to query comparision file
		fp << mMRH[i].pt.phi << "," << mMRH[i].pt.theta << ","
		   << mMRH[i].pt.phi/pi << "," << cos(mMRH[i].pt.theta) << ","
		   << mMRH[i].rec << "\n";

		foundMRH.push_back(mMRH[i]);

	}

	// Write out list of HPX Data Points
	fp << "\n\n";
	fp << "############################################\n";
	fp << "###### List of data point id's in HPX ######\n";
	fp << "############################################\n";
	fp << "Phi,Theta,P,Z,Rec\n";
	for( i = 0; i < mHPX.Npix(); i++)
	{
		if( mHPX[i].rec != EMPTY ) {
			fp	<< mHPX[i].pt.phi << "," << mHPX[i].pt.theta << ","
				<< mHPX[i].pt.phi/pi << "," << cos(mHPX[i].pt.theta) << ","
				<< mHPX[i].rec << "\n";
			foundHPX.push_back(mHPX[i]);
		}

	}

	AnalyzeResults(foundHPX,foundMRH,Matches,HPXUnique,MRHUnique,false);
	cout << " Matches: " << Matches << " MRH Only: " << MRHUnique << " HPX Only: " << HPXUnique << endl;
	fp << " Matches: " << Matches << " MRH Only: " << MRHUnique << " HPX Only: " << HPXUnique << "\n";
...
void ScrubDataSet(std::string dataInputFile, std::string outFile, int order)
{
	ofstream fp;
	ifstream ip;
	int i;
	...
	cout << "########################################" << endl;
	cout << "#### SCRUB MEASUREMENT DATA VIA HPX ####" << endl;
	cout << "########################################" << endl << endl;
	...
	for( i = 0; i < ms.size(); i++)
	{
		...
		// Check if HPX Map location is empty
		if( mHPX[hpxid].rec != EMPTY ) {
			cout << "Found " << mHPX[hpxid].rec << " at HPXID " << hpxid << ", Overwriting!" << endl;
		}
    ...
    // Open Scubbed Measurements Output File
	fp.open(outFile.c_str(),'w');
	if(fp.fail()){
	  cout << "Unable to open " << outFile.c_str() << "!" << endl;
	  exit(1);     
	}

	//Count number of non-empty cells in final HPX data structure
	nPts = 0;
	for( i = 0; i < mHPX.Npix(); i++ ) {
		if( mHPX[i].rec != EMPTY ) {
			nPts += 1;
	...
	cout << "Found " << nPts << " points in HPX data structure.\n";

	// Now output the contents of non-empty cells of HPX data structure 
	fp << nPts << "\t" << order << "\n";
	fp << "REC\tPHI\tTHETA\tD1\tD2\tD3\tD4\tV1\tV2\tV3\tV4\n";
	for( i = 0; i < mHPX.Npix(); i++ ) {
		if( mHPX[i].rec != EMPTY ) {
			fp << mHPX[i].rec << "\t";
			fp << std::setprecision(17) << mHPX[i].pt.phi << "\t";
			fp << std::setprecision(17) << mHPX[i].pt.theta << "\t";
			fp << std::setprecision(17) << mHPX[i].data1 << "\t";
			fp << std::setprecision(17) << mHPX[i].data2 << "\t";
			fp << std::setprecision(17) << mHPX[i].data3 << "\t";
			fp << std::setprecision(17) << mHPX[i].data4 << "\t";
			fp << std::setprecision(17) << mHPX[i].val1 << "\t";
			fp << std::setprecision(17) << mHPX[i].val2 << "\t";
			fp << std::setprecision(17) << mHPX[i].val3 << "\t";
			fp << std::setprecision(17) << mHPX[i].val4 << "\n";
		}		
	}
	fp.close();
}

void MRHExperimentalPointInsertion(std::string dataFile)
{
	ifstream ip;
	int i;
	int64 hpxid,measurements_order;
	...
	MultiResHpx_Map<Measurement> mMRH(measurements_order,NEST);
	for( i = 0; i < ms.size(); i++)
	{
		if( i%1000 == 0 ) {
			cout << "	Inserting Point " << i+1 << " of " << ms.size() << endl;
		}
		mMRH.AddRecord2(ms[i],ms[i].pt);
	}
...

{
	...
	int testNum, numPoints, numQueries, maxDepth, numTrials,duplicateYN, maxDepthMRH,maxDepthHPX;
	int minIdx,maxIdx,minNeighbors;
	int64 order;
	std::string outFile,inFile,dataFile,queryFile,confirmFile;
	float density;
	...
	if( testNum == -7 ) {
		...
		cout << "Outfile: " << outFile << endl;
		cout << "Min Neighbors: " << minNeighbors;
		CreateRandomNeighborsFile(numQueries,dataFile,outFile,minNeighbors);
	}
	...
	if( testNum == -12 ) {
		...
		MRHPointInsertionAndSave(dataFile,outFile,confirmFile,maxDepthMRH);
	}

	// Scrub Datasets via HPX datastructure final population
	if( testNum == -13 ) {
		dataFile = argv[2];
		outFile = argv[3];
		maxDepthHPX = atoi(argv[4]);
		ScrubDataSet(dataFile,outFile,maxDepthHPX);
	}

	// Test Experimental MRH Point Insertion Methodology
	if( testNum == -14 ) {
		dataFile = argv[2];
		MRHExperimentalPointInsertion(dataFile);
	}

	// Get Disc Query HPX Coverage Map Cell Counts
	if( testNum == -15 ) {
		queryFile = argv[2];
		DiscQueryHpxCoverageMapCellCount(queryFile);
	}

	// Get Poly Query HPX Coverage Map Cell Counts
	if( testNum == -16 ) {
		queryFile = argv[2];
		PolyQueryHpxCoverageMapCellCount(queryFile);
	}
	...
	if( testNum == 1 ) {
		...
		maxDepthMRH = atoi(argv[5]);
		MRHPointInsertionBenchMark(dataFile,outFile,numTrials,maxDepthMRH);
	}
	...
	if( testNum == 3 ) {
		...
		maxDepthMRH = atoi(argv[6]);
		MRHDiscQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials,maxDepthMRH);
	}
	...
	if( testNum == 4 ) {
		...
		outFile = argv[4];
		numTrials = atoi(argv[5]);
		HPXDiscQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials);
	}
	...
	if( testNum == 5 ) {
		...
		outFile = argv[4];
		maxDepthMRH = atoi(argv[5]);
		CompareDiscQueryOutput(dataFile,queryFile,outFile,maxDepthMRH);
	}
	...
	if( testNum == 6 ) {
		...
		numTrials = atoi(argv[5]);
		maxDepthMRH = atoi(argv[6]);
		MRHPolyQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials,maxDepthMRH);
	}
	...
	if( testNum == 7 ) {
		...
		outFile = argv[4];
		numTrials = atoi(argv[5]);
		HPXPolyQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials);
	}
	...
	if( testNum == 8 ) {
		...
		outFile = argv[4];
		maxDepthMRH = atoi(argv[5]);
		ComparePolyQueryOutput(dataFile,queryFile,outFile,maxDepthMRH);
	}
	...
	if( testNum == 9 ) {
		...
		numTrials = atoi(argv[5]);
		maxDepthMRH = atoi(argv[6]);
		MRHStripQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials,maxDepthMRH);
	}
	...
	if( testNum == 10 ) {
		...
		outFile = argv[4];
		numTrials = atoi(argv[5]);
		HPXStripQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials);
	}
	...
	if( testNum == 11 ) {
		...
		outFile = argv[4];
		maxDepthMRH = atoi(argv[5]);
		CompareStripQueryOutput(dataFile,queryFile,outFile,maxDepthMRH);
	}
	...
	if( testNum == 12 ) {
		...
		numTrials = atoi(argv[5]);
		maxDepthMRH = atoi(argv[6]);
		MRHNeighborQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials,maxDepthMRH);
	}
	...
	if( testNum == 13 ) {
		...
		outFile = argv[4];
		numTrials = atoi(argv[5]);
		HPXNeighborQueryBenchMarkV2(dataFile,queryFile,outFile,numTrials);
	}
	...
	if( testNum == 14 ) {
		...
		outFile = argv[4];
		maxDepthMRH = atoi(argv[5]);
		CompareNeighborQueryOutput(dataFile,queryFile,outFile,maxDepthMRH);
	}


	// HPX vs MRH Neighbor Query Accuracy Test USING SPECIFIED ORDER
	if( testNum == 15 ) {
		dataFile = argv[2];
		queryFile = argv[3];
		outFile = argv[4];
		order = atoi(argv[5]);
		CompareNeighborQueryOutputAtOrder(dataFile,queryFile,outFile,order);
	}
	...
	if( testNum == 17 ) {
		...
		outFile = argv[3];
		maxDepthMRH = atoi(argv[4]);
		CompareDataStructures(dataFile,outFile,maxDepthMRH);
	}
