...
template<typename I> inline I T_Healpix_Base<I>::xyf2nest (int ix, int iy,
  int fn) const
  { 
	  return (I(fn)<<(2*order_)) + spread_bits(ix) + (spread_bits(iy)<<1); 
  }
...
template<typename I> void T_Healpix_Base<I>::ring2xyf (I pix, int &ix, int &iy,
  ...
  if (pix<ncap_) // North Polar cap
    ...
    fn = (ifp==ifm) ? (ifp|4) : ((ifp<ifm) ? ifp : (ifm+8));
    }
  ...
  I irt = iring - (jrll[fn]*nside_) + 1;
  I ipt = 2*iphi- jpll[fn]*nr - kshift -1;
...
template<typename I> I T_Healpix_Base<I>::xyf2ring (int ix, int iy,
  ...
  nr>>=2;
  kshift=1-shifted;
  I jp = (jpll[fn]*nr + ix - iy + 1 + kshift) / 2;
...
template<typename I> I T_Healpix_Base<I>::pix2ring (I pix) const
  ...
  if (scheme_==RING)
    ...
    {
    int fn, ix, iy;
    nest2xyf(pix,ix,iy,fn);
    return (I(jrll[fn])<<order_) - ix - iy - 1;
    }
  }
...
template<typename I> I T_Healpix_Base<I>::ring2nest (I pix) const
  ...
  planck_assert(order_>=0, "hierarchical map required");
  int ix, iy, fn;
  ring2xyf (pix, ix, iy, fn);
  return xyf2nest (ix, iy, fn);
  }
...
template<typename I> I T_Healpix_Base<I>::loc2pix (double z, double phi,
  ...
  if (scheme_==RING)
    ...
    if (za<=twothird) // Equatorial region
      ...
      I ifm = jm >> order_;
      int fn = (ifp==ifm) ? (ifp|4) : ((ifp<ifm) ? ifp : (ifm+8));
...
template<typename I> void T_Healpix_Base<I>::neighbors (I pix,
  ...
  int ix, iy, fn;
  (scheme_==RING) ?
    ring2xyf(pix,ix,iy,fn) : nest2xyf(pix,ix,iy,fn);
  ...
  if ((ix>0)&&(ix<nsm1)&&(iy>0)&&(iy<nsm1))
    ...
    if (scheme_==RING)
      for (int m=0; m<8; ++m)
        result[m] = xyf2ring(ix+nb_xoffset[m],iy+nb_yoffset[m],fn);
    else
      {
      I fpix = I(fn)<<(2*order_),
...
template<typename I> int T_Healpix_Base<I>::GetCritCount()
{
	return CRITICAL_COUNT_;
}

template<typename I> void T_Healpix_Base<I>::ResetCritCount()
{
	CRITICAL_COUNT_ = 0;
}
