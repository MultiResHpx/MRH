...
// Build MultiResHpx Data Structure
	...
	cout << std::setprecision(20) << "	Completed " << ms.size() << " MRH Point Insertions in " << double(stop_s-start_s) 
		<< " ms! Average MRH Point Insertion: " << avgPtInsertTime << " ms\n";

	// Write out Trial Result to output file
	fp << std::setprecision(20) << "	Completed " << ms.size() << " MRH Point Insertions in " << double(stop_s-start_s) 
		<< " ms! Average MRH Point Insertion: " << avgPtInsertTime << " ms\n";
...

{
	...
	bool verbose;
	std::string outFile,outFile2,outPath,dataFile,queryFile;
	std::vector<std::string> queryFilesList;
	...
	if( testNum == 3 ) {
		...
		queryFilesList.push_back(argv[3]); // Disc Query File
		queryFilesList.push_back(argv[4]); // Poly Query File
		queryFilesList.push_back(argv[5]); // Strip Query File
		queryFilesList.push_back(argv[6]); // Neighbors Query File
		outFile = argv[7];
		outPath = argv[8];
		numTrials = atoi(argv[9]);
		verbose01 = atoi(argv[10]);
		if(verbose01 == 1) verbose = true;
		maxDepthMRH = atoi(argv[11]);
		MRHBenchMarkFINAL(dataFile,queryFilesList,outFile,outFile2,outPath,numTrials,verbose,maxDepthMRH);
	}
	...
	if( testNum == 4 ) {
		...
		outFile = argv[7];
		outPath = argv[8];
		verbose01 = atoi(argv[9]);
		maxDepthMRH = atoi(argv[10]);
		if(verbose01 == 1) verbose = true;
		MRHvsHPXComparisionFINAL(dataFile,queryFilesList,outFile,outPath,verbose,maxDepthMRH);
	}
}
